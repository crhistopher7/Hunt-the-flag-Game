...
public class PlayerController : MatchBehaviour
{
    ...
    {
        PathfinderObj = GameObject.Find("Pathfinder").GetComponent<AStar>();
    ...
    private List<LogicMap> RunP4Code(LogicMap current, LogicMap objective, LogicMap deceptive, PathType pathType, string MAP_HEIGHTMAP_FILE)
    {
        ...
        UnityEngine.Debug.Log("Sa�da do Python: \n" + output);
        UnityEngine.Debug.Log("Sa�da de erro do Python: \n" + errorOutput);

        List<LogicMap> logicMapList = new List<LogicMap>();
        if (output.Contains("FULL PATH"))
        {
            string listaString = output.Split(':').Last();
            UnityEngine.Debug.Log(listaString);
            ...
            foreach (Match match in matches)
            {
                ...
                LogicMap point = PathfinderObj.GetTileByPosition(new Vector3Int(Constants.CLICK_POSITION_OFFSET + x, Constants.CLICK_POSITION_OFFSET + y, 0));
